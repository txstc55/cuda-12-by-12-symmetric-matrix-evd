...
int main() {
    ...
    unsigned int selected_index = 999999;
    ...
    std::vector<double> values =
      readVectorFromFile<double>("given_values.txt");
    batchSize = values.size() / (n * n);
    printf("Batch size: %d\n", batchSize);
    ...
    cudaMemcpy(d_A, values.data(), sizeof(double) * n * n * batchSize, cudaMemcpyHostToDevice);
